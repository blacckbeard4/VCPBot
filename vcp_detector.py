...
def detect_vcp(ticker: str, df: pd.DataFrame) -> tuple[Optional[dict], str]:
    ...
    if base_weeks < MIN_BASE_WEEKS:
        ...
        return None, f"base too short ({base_weeks}w < {MIN_BASE_WEEKS}w required)"
    ...
    if bars_since_year_high < 20:
        return None, "too few bars in base (<20 bars)"
    ...
    if len(contractions) < MIN_CONTRACTIONS:
        ...
        return None, f"too few contractions ({len(contractions)} found, need {MIN_CONTRACTIONS})"
    ...
    for i in range(1, len(depths)):
        if depths[i] >= depths[i - 1]:
            ...
            return None, f"not tightening: contraction {i} depth {depths[i]:.1%} >= prior {depths[i-1]:.1%}"
    ...
    if final_depth >= MAX_FINAL_CONTRACTION_PCT:
        ...
        return None, f"final contraction {final_depth:.1%} >= {MAX_FINAL_CONTRACTION_PCT:.0%} max"

    last_contraction = recent[-1]
    sh_idx = last_contraction["sh_idx"]
    sl_idx = last_contraction["sl_idx"]

    # ── 7. Volume dry-up in the final contraction ──
    final_range_volume = base_volume.iloc[sh_idx:sl_idx + 1]
    if len(final_range_volume) < 1:
        return None, "empty final contraction volume range"
    ...
    if not volume_dry_up:
        ...
        return None, f"no volume dry-up (final avg {final_avg_vol:,.0f} vs 50d avg {avg_vol_50d:,.0f})"
    ...
    if stop_pct > MAX_STOP_PCT:
        ...
        return None, f"stop {stop_pct:.1%} below pivot (max {MAX_STOP_PCT:.0%})"
...
def detect_vcp_batch(scan_results: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    Returns (vcp_setups, rejections):
      vcp_setups: list of dicts combining scan fields with VCP detection results.
      rejections: list of {ticker, phase, reason} dicts for failed VCP checks.
    """
    ...
    for stock in scan_results:
        ...
        try:
            ...
            rejections.append({"ticker": ticker, "phase": "VCP", "reason": f"exception: {e}"})
